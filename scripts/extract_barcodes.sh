...
echo "saving output in barcodes_${FNAME}.txt"
...
function cb_on_chrom() {
       samtools view $1 $2 | grep -Po "CB:Z:[A|G|C|T]+-1" | sort -uo "$tmp_dir/$2.txt"
}
...
cat $tmp_dir/*.txt | sort --parallel $MAX_FORKS -uo barcodes_$FNAME.txt
rm -rf $tmp_dir	

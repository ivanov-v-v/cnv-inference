...
def total_loglikelihood(A_DNA, R_DNA, X_DNA, logbincoeffs_DNA,
    ...
    loglik_DNA = np.mean(logits_DNA[np.isfinite(logits_DNA)])
    loglik_RNA = np.mean(logits_RNA[np.isfinite(logits_RNA)])
...
def predict_cell_label(cell_id, A, R, logbincoeffs, X_CLONE, f):
    ...
    return random_choice_nb(np.arange(probas.size), probas)
...
def simulate_G_T(tau, T_mat, D_RNA_seed, D_DNA_seed, n_cell, random_seed=None):
        ...
        idx_D = np.random.choice(D_DNA_seed.shape[1], size=n_cell)

        D_RNA = D_RNA_seed[:, idx_D].astype(int)
        D_DNA = D_DNA_seed[:, idx_D].astype(int)
...
#                X_DNA[i, j] = Theta[i, int(T_mat[i, I_DNA[j]])]
        ...
        H_DNA, X_DNA = init_H_X(N=n_block, M=n_cell, T=T_mat, I=I_DNA, Theta=Theta)
        A_DNA = np.random.binomial(D_DNA, X_DNA)
        A_DNA[~np.isfinite(A_DNA)] = 0
        A_DNA[A_DNA < 0] = 0
        R_DNA = D_DNA - A_DNA

        H_RNA, X_RNA = init_H_X(N=n_block, M=n_cell, T=T_mat, I=I_DNA, Theta=Theta)
        A_RNA = np.random.binomial(D_RNA, X_RNA)
        A_RNA[~np.isfinite(A_RNA)] = 0
        A_RNA[A_RNA < 0] = 0
        R_RNA = D_RNA - A_RNA
        ...
        assert np.all(A_DNA <= D_DNA)
        assert np.all(A_RNA <= D_RNA)
        assert np.all(A_CLONE <= D_CLONE)
        
        H_CLONE, X_CLONE = init_H_X(N=n_block, M=n_clone, T=T_mat, I=np.arange(n_clone), Theta=Theta)
        post_Alpha, post_Beta, post_Theta = Alpha.copy(), Beta.copy(), Theta.copy()

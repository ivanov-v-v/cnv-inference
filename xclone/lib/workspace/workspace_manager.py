...
class WorkspaceManager:
    def __init__(
        ...
        if cookiecutter_info is None:
            cookiecutter_info = xclone.lib.system_utils.load_cookiecutter_info(xclone_config)
    ...
    def load_data(self, requirements):
        return {
            dtype :
            xclone.lib.system_utils.pickle_load(self.tmp_data[dtype])
            for dtype in tqdm(
                requirements,
    ...
    def write_config(self):
        os.makedirs(f"{self._nbconfig['config_dir']}/workspaces", exist_ok=True)
        outfile = "{}/workspaces/{}_{}_workspace.pkl".format(
            self._nbconfig['config_dir'],
            self._nbconfig["experiment_info"]["sample"],
            self._nbconfig['experiment_info']["modality"]
        )
        xclone.lib.system_utils.pickle_dump(self._nbconfig, outfile)

    def load_config(self):
        infile = "{}/workspaces/{}_{}_workspace.pkl".format(
            self._nbconfig['config_dir'],
            self._nbconfig["experiment_info"]["sample"],
            self._nbconfig['experiment_info']["modality"]
        )
        self._nbconfig = xclone.lib.system_utils.pickle_load(infile)
    ...
    def _upload_tmp_data(self, requirements):
        ...
        for dtype in tqdm(requirements.keys(), "copying files"):
            print("{} —> {}".format(
